...
- `/agents thinking`: show the current session thinking mode, enable it with `/agents thinking enable`, disable it with `/agents thinking disable`, or reset to the agent default with `/agents thinking reset`; changing the mode starts a fresh session so the control reply does not contaminate the next model prompt while preserving other session settings
